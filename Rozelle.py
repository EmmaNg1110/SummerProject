...
# -*- coding: utf-8 -*-
"""
Created on Tue Jan  2 15:53:20 2018
...
#Seperating sites into Rozelle
new6 = df.loc[df['site'] == 'Rozelle']

#Monthly mean of Rozelle
...
new6 = new6.to_period(freq='M')


# Tail-rolling average transform for O3

for i in range (1,13): 
    rolling = new6[['O3']][new6.mon==i].rolling(window = 3, center=True)
    ...
    new6['O3'][new6.mon==i].plot(label='Raw')
    
    #anomoly = raw data - moving average 
    anomoly = (new6['O3'][new6.mon==i]) - rolling_mean['O3']
    
    anomoly.plot(label='Anomaly')
    
    #print(anomoly,new6['O3'][new6.mon==i], rolling_mean['O3'])
    ...
    pyplot.legend()
    pyplot.title(i)
    pyplot.savefig("Rozelle_O3_Month" + str(i) + ".png")
    pyplot.show()
...
pyplot.title('Rozelle_O3')
pyplot.savefig("Rozelle_O3_Allmonths.png")
pyplot.show()


# Tail-rolling average transform for PM2.5
for j in range (1,13): 
    rolling2 = new6[['PM2.5']][new6.mon==j].rolling(window = 3, center=True)
    ...
    new6['PM2.5'][new6.mon==j].plot(label='Raw')
    
    #anomoly = raw data - moving average 
    anomoly2 = (new6['PM2.5'][new6.mon==j]) - rolling_mean2['PM2.5']
    
    anomoly2.plot(label='Anomaly')
    
    #print(anomoly,new6['PM2.5'][new6.mon==i], rolling_mean['PM2.5'])
    ...
    pyplot.legend()
    pyplot.title(j)
    pyplot.savefig("Rozelle_PM2.5_Month" + str(j) + ".png")
    pyplot.show()
...
pyplot.grid(True)
pyplot.legend(loc='best')
pyplot.title('Rozelle_PM2.5')
pyplot.savefig("Rozelle_PM2.5_Allmonths.png")
