...
# -*- coding: utf-8 -*-
"""
Created on Wed Jan  3 15:29:21 2018
...
Bargo_O3 = df['BARGO OZONE']*10.0

plt.plot(Bargo_O3, '.',color='g')
...
plt.plot(df_mm['BARGO OZONE'], '.',color='g')
plt.show()
...
new1['BARGO OZONE'] = new1['BARGO OZONE']*10.0

for i in range (1,13): 
    rolling = new1[['BARGO OZONE']][new1.mon==i].rolling(window = 3, center=True)

    rolling_mean = rolling.mean()
    
    # plot original and transformed dataset
    
    rolling_mean['BARGO OZONE'].plot(label='Moving ave')
    
    #Index only O3 column, then mon==1 is just getting the Jan column from that 
    new1['BARGO OZONE'][new1.mon==i].plot(label='Raw')
    
    #anomoly = raw data - moving average 
    anomoly = (new1['BARGO OZONE'][new1.mon==i]) - rolling_mean['BARGO OZONE']
    ...
    plt.legend()
    plt.title(i)
    plt.savefig("BARGO_O3_Month" + str(i) + ".png")
    plt.show()
    
#Putting all months together
    if i == 1:
        al1 = anomoly
        avg1 = rolling_mean[['BARGO OZONE']]
        raw1 =  new1['BARGO OZONE'][new1.mon==i]
    else:
        temp = anomoly
        al1 = pd.concat([al1, temp])
        
        temp = rolling_mean[['BARGO OZONE']]
        avg1 = pd.concat([avg1, temp])
        
        temp = new1['BARGO OZONE'][new1.mon==i]
...
plt.legend(loc='best')
plt.title('BARGO_O3')
plt.savefig("BARGO_O3_Allmonths.png")
plt.show()
